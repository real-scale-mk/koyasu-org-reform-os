...
Pane4 **最下部** に固定表示。ステークホルダー表の下、ペイン内フッターとして配置。v1 は静的テキスト（JSON固定・編集UIなし）。

- 問題を管理するのではなく、**構造を見つける**ために画面を設計した
- **現象選択 → ハザード地図 → 因果ループ → 次の一手** の連動が、コンサルタントの思考の流れそのものだった
- 機能を足すより **「何を作らないか」** を決めるほうが、提出品質と説明力が上がった
- AIは表・モーダルに逃げがち。**地図とループ** を仕様で先に固定する必要があった
- **責任曖昧が育成不足の起点である可能性**
